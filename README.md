...
优先使用 LLDB 动态命令：FreeCAD/OCCT 在断点暂停时，开发者可以直接执行：
...
Capture 动态库只需一次构建和加载；之后观察不同变量不需要修改源码或重新编译。高频循环、极短生命周期和异常前关键状态才使用源码探针。
...
渲染逻辑不依赖生产方式。完整设计见 [Kit 的 LLDB 动态几何采集文档](https://github.com/liyongzheng666/freecad-occt-debug-kit/blob/main/docs/lldb-dynamic-geometry-capture.md)。
...
3. M2：LLDB 动态 Capture、BREP/mesh、FCStd baseline 和世界坐标对齐。

...
const meshRenderer: EntityRenderer = {
  create(entity) {
    ...
    if (!geometry?.positions?.length || !geometry.indices?.length) return null;
    const buffer = new BufferGeometry();
...
// is required for the dash pattern.
const bboxRenderer: EntityRenderer = {
  create(entity) {
    ...
    const [x0, y0, z0] = geometry.min;
    const [x1, y1, z1] = geometry.max;
    const corner: Vec3[] = [
